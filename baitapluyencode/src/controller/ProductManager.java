...
public class ProductManager extends ProductList implements OptionProduct {
    public void addProduct(String name, int id, int price, boolean status, String description){
        Product product = new Product(name, id, price, status, description);
        productList.add(product);

    }
    @Override
    public void edit(int index,String name, int id, int price, boolean status, String description) {
    ...
    public void search(String name) {
        for (int i = 0;i < productList.size();i++){
            if(productList.get(i).getName() == name){
                ...
                System.out.println("Trạng thái : " + productList.get(i).isStatus());
    ...
    public void soft(boolean check) {
        ...
        if(check){
            for (int key : softProduct.keySet()){
                ...
                System.out.println("Trạng thái : " + softProduct.get(key).isStatus());
